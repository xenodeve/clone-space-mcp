...
| 6.4 | **Capability flags** — service-worker-dependent · WebSocket-dependent · closed shadow root present · OOPIF not instrumented · no sourcemap | Without them `extract` returns an empty graph that reads as success. Adding them after the tool manifest ships breaks every consumer |
| 6.5 | **Request normalization + unexpected-request taxonomy** | Playwright matches HAR strictly on URL, method and body, so a nonce, timestamp, CSRF token or UUID query parameter aborts a logically identical request. **This is the real time-and-randomness failure**, not the id counter |
...
| 6.4 | **capability flag** — ต้องใช้ service worker · ต้องใช้ WebSocket · มี closed shadow root · มี OOPIF ที่ไม่ได้ instrument · ไม่มี sourcemap | ถ้าไม่มี `extract` จะคืน graph ว่างที่อ่านแล้วเหมือนสำเร็จ การเพิ่มทีหลังจาก tool manifest ออกไปแล้วทำให้ผู้ใช้ทุกรายพัง |
| 6.5 | **การ normalize request และการจัดหมวด request ที่ไม่คาดคิด** | Playwright จับคู่ HAR แบบเข้มด้วย URL, method และ body ดังนั้น nonce, timestamp, CSRF token หรือ UUID ใน query parameter จะทำให้ request ที่เป็นอันเดียวกันในเชิงตรรกะถูก abort **นี่คือความล้มเหลวเรื่องเวลาและการสุ่มตัวจริง** ไม่ใช่ตัวนับ id |
