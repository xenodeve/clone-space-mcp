...
| Item | Status | Gate | Next action |
|---|---|---|---|
| #171 Slice 0 — equivalence gate | ✅ | merged (#172, #207) | `classify` + `run`; verdicts `equal / allowed / different / unobserved / unstable`, coverage as a vector. Carries the #182 stability work merged through #183. **This row said ✅ while two acceptance criteria were unmet** — the issue stayed open and the row did not say why, which is the same shape as #2 being closed with its condition live. Criterion 5 landed 2026-08-17 (PR #207): `bun run equivalence <url>`, three exit codes, and the module finally has an entry point outside `bun test`. Criterion 7's three recorded verdicts are in [`docs/reports/2026-08-17-equivalence-verdicts.md`](reports/2026-08-17-equivalence-verdicts.md) — `labs.chaingpt.org` gave **PASS then FAIL minutes apart**, reproducing #182 on the first day the command existed |
