...
| #162 Private-address content can reach a published archive | ✅ | pending merge | `captureHar` refuses to publish when a HAR entry's `serverIPAddress` is private, unless `allowPrivateNetwork` (#162 branch). Closes all three vectors with one rule because the address is a fact about the connection, not a lookup. **Two follow-ons deliberately not taken:** the range table covers neither CGNAT `100.64.0.0/10` nor multicast, and it is shared with the pre-flight guard so widening it changes shipped behaviour; and the refusal discards the whole archive rather than the offending entry, so one leftover loopback beacon fails a public capture |
...
| **#182 The gate's verdict is not reproducible** | 🟡 | partial fix merged | **The mechanism this repo trusts to judge clone fidelity cannot be trusted yet.** Three runs on one unchanged site gave FAIL, PASS and INCOMPLETE. Five approaches measured, four refuted by building them. What merged makes unreproducible fields report as *unstable* instead of *accusing the clone*; the verdict still varies. Until it closes, a gate result is one sample and a `FAIL` must be re-run before anyone acts on it |
