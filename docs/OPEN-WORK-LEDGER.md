...
| **#185 A WebSocket carries no `serverIPAddress`, so #162's rule cannot see it** | 🟡 | filed | Measured on the fixture: the WS entry has `_resourceType: "websocket"` and no address, while the document and XHR entries beside it carry `"[::1]"`. Its only signal is the URL host, which is the name-based check #162 exists because it could not answer — so this is a policy decision, not a classifier gap. Option 3 in the issue (read the remote address from CDP) is the one that removes the special case instead of adding one, and it needs a measurement first |
