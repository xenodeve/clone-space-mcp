...
| #182 The gate's verdict is not reproducible | 🟡 | PR #183 is partial | **The mechanism this repo trusts to judge clone fidelity cannot be trusted yet.** Three runs on one unchanged site gave FAIL, PASS and INCOMPLETE. Five approaches measured, four refuted by building them; the fifth — stability measured on **both** sides rather than only live — is in PR #183 and converges without terminating. Until it closes, a gate result is one sample and a `FAIL` must be re-run before anyone acts on it |
