...
| **#187 Two replays of one archive lay out to different heights** | 🔴 | filed | 8544 live on every load; 8544, 8486, 8486 across three replays of one archive, constant within each run. `dom.elements` moves with it, 2858 against 2860. Not sampling, not the archive, not the network. The first thing to settle is whether the two missing elements cause the height or follow from it |
